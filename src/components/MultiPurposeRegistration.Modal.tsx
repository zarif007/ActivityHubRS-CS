...
const MultiPurposeRegistrationModal = ({
  isOpen,
  setIsOpen,
  options,
  dedicated,
  ...
  const validateInputs = () => {
    if (registrationInputs.id === "") {
      setError(
        `You must select ${isSeminar(options[0]) ? "Seminar" : "Workshop"}`
      );
      return false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // For now, will change for seminar later
    if (isSeminar(options[0]) || !studentInfo?._id) return;

    try {
      const { data } = await axios.post(
        `${apiEndpointV1}/workshop/register/${registrationInputs.id}`,
                ...
                <div className="w-full">
                  ...
                  >
                    ...
                    <div className="w-full sm:px-6">
                      {studentInfo && (
                        <StudentInfoDisplayer studentInfo={studentInfo} />
                      )}
                    </div>
